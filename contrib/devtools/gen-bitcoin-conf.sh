...
LITECOIND=${LITECOIND:-$BINDIR/litecoind}
SHARE_EXAMPLES_DIR=${SHARE_EXAMPLES_DIR:-$TOPDIR/share/examples}
EXAMPLE_CONF_FILE=${EXAMPLE_CONF_FILE:-$SHARE_EXAMPLES_DIR/litecoin.conf}

[ ! -x "$LITECOIND" ] && echo "$LITECOIND not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($LITECOIND --version)
...
echo 'Generating example litecoin.conf file in share/examples/'
...
# parse the output from litecoind --help
