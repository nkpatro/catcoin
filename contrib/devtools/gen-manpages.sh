...
LITECOIND=${LITECOIND:-$SRCDIR/flamecoind}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/flamecoin-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/flamecoin-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/flamecoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
FLAVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
