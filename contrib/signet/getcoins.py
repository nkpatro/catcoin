...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing litecoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='litecoin-cli', help='litecoin-cli command to use')
...
parser.add_argument('litecoin_cli_args', nargs='*', help='Arguments to pass on to litecoin-cli (default: -signet)')

args = parser.parse_args()

if args.litecoin_cli_args == []:
    args.litecoin_cli_args = ['-signet']


def litecoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.litecoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = litecoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = litecoin_cli(['getnewaddress', 'faucet', 'bech32'])
