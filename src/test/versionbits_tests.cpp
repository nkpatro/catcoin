...
static void check_computeblockversion_bip8(VersionBitsCache& versionbitscache, const Consensus::Params& params, Consensus::DeploymentPos dep)
{
...

    {
        ...
        BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & bit_mask, 0);
        ...
        for (uint32_t i = nStartHeight; i < nTimeoutHeight; i++) {
            ...
            BOOST_CHECK((versionbitscache.ComputeBlockVersion(lastBlock, params) & bit_mask) != 0);
            BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
            BOOST_CHECK_EQUAL(versionbitscache.State(lastBlock, params, dep), ThresholdState::STARTED);
        }
        ...
        for (uint32_t i = 0; i < nMinerConfirmationWindow; i++) {
            ...
            BOOST_CHECK((versionbitscache.ComputeBlockVersion(lastBlock, params) & bit_mask) != 0);
            BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
            BOOST_CHECK_EQUAL(versionbitscache.State(lastBlock, params, dep), ThresholdState::LOCKED_IN);
        }
        ...
        BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & bit_mask, 0);
        BOOST_CHECK_EQUAL(versionbitscache.State(lastBlock, params, dep), ThresholdState::ACTIVE);
    }

    {
        ...
        lastBlock = chain.Mine(nStartHeight - 1, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
        BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & bit_mask, 0);
        BOOST_CHECK_EQUAL(versionbitscache.State(lastBlock, params, dep), ThresholdState::DEFINED);
...
static void check_computeblockversion(VersionBitsCache& versionbitscache, const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    if (nStartTime == 0 && nTimeout == 0) {
        return check_computeblockversion_bip8(versionbitscache, params, dep);
    }
