    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional JOCO outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes JOCO and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::JOCO_TO_JOCO) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::JOCO_ONLY;
        ...
        // If JOCO-to-JOCO fails, create a peg-out transaction
