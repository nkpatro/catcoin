...
const int FIELDCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString FIELDCOIN_IPC_PREFIX("fieldcoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(FIELDCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(FIELDCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(FIELDCOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click fieldcoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start fieldcoin: click-to-pay handler");
    else
...
{
    // clicking on fieldcoin: URLs creates FileOpen events on the Mac:
