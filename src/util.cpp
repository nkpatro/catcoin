...
const char * const BITCOIN_CONF_FILENAME = "khmerriel.conf";
const char * const BITCOIN_PID_FILENAME = "khmerriel.pid";
...
#else
    const char* pszModule = "khmerriel";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KhmerRiel";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/KhmerRiel";
#else
    // Unix
    return pathRet / ".khmerriel";
#endif
#endif
}
