...
const char * const BITCOIN_CONF_FILENAME = "jeanjohncoin.conf";
const char * const BITCOIN_PID_FILENAME = "jeanjohncoind.pid";
...
#else
    const char* pszModule = "jeanjohncoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JeanJohnCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/JeanJohnCoin";
#else
    // Unix
    return pathRet / ".jeanjohncoin";
#endif
#endif
}
