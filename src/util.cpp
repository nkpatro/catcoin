...
const char * const BITCOIN_CONF_FILENAME = "flamecoin.conf";
const char * const BITCOIN_PID_FILENAME = "flamecoind.pid";
...
#else
    const char* pszModule = "flamecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Flamecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Flamecoin";
#else
    // Unix
    return pathRet / ".flamecoin";
#endif
#endif
}
