...
const char * const BITCOIN_CONF_FILENAME = "scpfoundation.conf";
const char * const BITCOIN_PID_FILENAME = "scpfoundationd.pid";
...
#else
    const char* pszModule = "scpfoundation";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SCPFoundation";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SCPFoundation";
#else
    // Unix
    return pathRet / ".scpfoundation";
#endif
#endif
}
