...
{
    // Fieldcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Fieldcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Fieldcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Fieldcoin: 2.5 minutes
...
void ThreadScriptCheck() {
    RenameThread("fieldcoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Fieldcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Fieldcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Fieldcoin: increase each by adding 2 to fieldcoin's value.
...
void static FieldcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running FieldcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FieldcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateFieldcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FieldcoinMiner, pwallet));
}
