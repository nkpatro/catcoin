...
uint256 hashGenesisBlock("0x38624e3834cfdc4410a5acbc32f750171aadad9620e6ba6d5c73201c16f7c8d1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FieldCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FieldCoin Signed Message:\n";
...
static const int64 nTargetTimespan = 60 * 60;						// FieldCoin: every 60 minutes
static const int64 nTargetSpacing = 60;								// FieldCoin: 60 sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // FieldCoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal fieldcoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "FieldCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xcf9688ea6a7c0b5bb7018d0339ba2e8011ac429a51caafb6a3f8f3fc7bf70a67");
    }
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // FieldCoind: Reduce the exempted free transactions to 500 bytes (from Fieldcoin's 3000 bytes)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FieldcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("FieldcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadFieldcoinMiner(void* parg);

static bool fGenerateFieldcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void static FieldcoinMiner(CWallet *pwallet)
{
    printf("FieldcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("fieldcoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateFieldcoins)
    {
        if (fShutdown)
            return;
        ...
        printf("Running FieldcoinMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadFieldcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        FieldcoinMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadFieldcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadFieldcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadFieldcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateFieldcoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateFieldcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateFieldcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d FieldcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadFieldcoinMiner, pwallet))
                printf("Error: CreateThread(ThreadFieldcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
