...
#if defined(NDEBUG)
# error "Calcoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Calcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Calcoin Signed Message:\n";
...
{
    // Calcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 4130000 * COIN;

    // Subsidy is cut in half every 25000 blocks, which will occur approximately every 34.7 days
    nSubsidy >>= (nHeight / 25000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Calcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Calcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Calcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Calcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Homestuck 01/Nov/2014 Caliborn Launches a CalCoin Powered Kickstarter Parody";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1414943290;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4019587262;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Calcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CalcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CalcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CalcoinMiner(CWallet *pwallet)
{
    printf("CalcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("calcoin-miner");
    ...
    try { loop {
        ...
        printf("Running CalcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CalcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CalcoinMiner, pwallet));
}
