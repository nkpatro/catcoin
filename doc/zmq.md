...
The Catcoin Core daemon can be configured to act as a trusted "border
router", implementing the catcoin wire protocol and relay, making
...
The ZeroMQ feature in Catcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building catcoind:
...
For instance:
    ...
    $ catcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/catcoind.tx.raw \
...
These options can also be provided in catcoin.conf.
...
From the perspective of catcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into catcoind directly. Furthermore, no information is
...
using. Catcoind appends an up-counting sequence number to each
