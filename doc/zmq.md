...
The Jocoin Core daemon can be configured to act as a trusted "border
router", implementing the jocoin wire protocol and relay, making
...
The ZeroMQ feature in Jocoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building jocoind:
...
For instance:
    ...
    $ jocoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/jocoind.tx.raw \
...
These options can also be provided in jocoin.conf.
...
From the perspective of jocoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into jocoind directly. Furthermore, no information is
...
using. Jocoind appends an up-counting sequence number to each
