UNIX BUILD NOTES
====================
Some notes on how to build Neoncoin Core in Unix.
...
memory available when compiling Neoncoin Core. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build Neoncoin Core without wallet.
...
If you want to build Neoncoin-Qt, make sure that the required packages for Qt development
...
# Configure Neoncoin Core to use our own-built instance of BDB
cd $LITECOIN_ROOT
./autogen.sh
...
As mentioned above, when maintaining portability of the wallet between the standard Neoncoin Core distributions and independently built
...
with 4.8-built Neoncoin Core is needed follow the steps under "Berkeley DB" above.
