Datachain Core version *0.15.1* is now available from:

  <https://download.datacoin.org/datacoin-0.15.1/>
...
A number of changes to the way Datachain Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 millidatacoins per kB (0.00001 DTC). This is to prepare for dropping the minimum transaction fee to 0.1 millidatacoins per kB (0.0001 DTC) in the next release.
