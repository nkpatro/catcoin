Flamecoin Core version *0.15.1* is now available from:

  <https://download.flamecoin.org/flamecoin-0.15.1/>
...
A number of changes to the way Flamecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 flametoshi per kB (0.00001 FLA). This is to prepare for dropping the minimum transaction fee to 0.1 flametoshi per kB (0.0001 FLA) in the next release.
