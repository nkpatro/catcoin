Tapcoin Core version *0.15.1* is now available from:

  <https://download.tapcoin.org/tapcoin-0.15.1/>
...
A number of changes to the way Tapcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 taps per kB (0.00001 TPC). This is to prepare for dropping the minimum transaction fee to 0.1 taps per kB (0.0001 TPC) in the next release.
