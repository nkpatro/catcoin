...
example /usr/src/vertexcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
This means you cannot use a directory that located directly on the host Windows file system to perform the build.

The next three steps are an example of how to acquire the source in an appropriate way.

    cd /usr/src
    sudo git clone https://github.com/vertexcoin-project/vertexcoin.git
    sudo chmod -R a+rw vertexcoin
...
way. This will install to `c:\workspace\vertexcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/vertexcoin
...
causes two of the vertexcoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the vertexcoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the vertexcoin source code).
