...
- [Building Vertexcoin Core](#building-vertexcoin-core)
...
Clone the git repositories for vertexcoin and Gitian.
...
in the vertexcoin repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the vertexcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/vertexcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/vertexcoin-project/vertexcoin
...
For example:
```bash
URL=https://github.com/thrasher-/vertexcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit vertexcoin=${COMMIT} --url vertexcoin=${URL} ../vertexcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit vertexcoin=${COMMIT} --url vertexcoin=${URL} ../vertexcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit vertexcoin=${COMMIT} --url vertexcoin=${URL} ../vertexcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the vertexcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../vertexcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/vertexcoin
SIGPATH=/some/root/path/vertexcoin-detached-sigs

./bin/gbuild --url vertexcoin=${BTCPATH},signature=${SIGPATH} ../vertexcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vertexcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vertexcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/vertexcoin-osx-build.assert
```
