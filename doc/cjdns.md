...
CJDNS peers than Tor or IP ones. You can use `litecoin-cli -addrinfo` to see the
...
To see which CJDNS peers your node is connected to, use `litecoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `litecoin-cli getpeerinfo`).
