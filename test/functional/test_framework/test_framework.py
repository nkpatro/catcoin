...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def setup(self):
        ...
        fname_litecoind = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "litecoind" + config["environment"]["EXEEXT"],
        )
        fname_litecoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "litecoin-cli" + config["environment"]["EXEEXT"],
        )
        fname_litecoinutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "litecoin-util" + config["environment"]["EXEEXT"],
        )
        self.options.litecoind = os.getenv("LITECOIND", default=fname_litecoind)
        self.options.litecoincli = os.getenv("LITECOINCLI", default=fname_litecoincli)
        self.options.litecoinutil = os.getenv("LITECOINUTIL", default=fname_litecoinutil)
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'litecoind', self.options.litecoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'litecoin-cli', self.options.litecoincli) for v in versions]
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as litecoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    litecoind=self.options.litecoind,
                    litecoin_cli=self.options.litecoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in litecoin.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if litecoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("litecoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if litecoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("litecoin-wallet has not been compiled")

    def skip_if_no_bitcoin_util(self):
        """Skip the running test if litecoin-util has not been compiled."""
        if not self.is_bitcoin_util_compiled():
            raise SkipTest("litecoin-util has not been compiled")
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether litecoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoin_util_compiled(self):
        """Checks whether litecoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_LITECOIN_UTIL")
