...
def main():
    ...
    tmpdir = "%s/scpfoundation_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and scpfoundationd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "scpfoundationd"]) is not None:
            print("%sWARNING!%s There is already a scpfoundationd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "SCPFOUNDATIOND" not in os.environ:
        os.environ["SCPFOUNDATIOND"] = build_dir + '/src/scpfoundationd' + exeext
        os.environ["SCPFOUNDATIONCLI"] = build_dir + '/src/scpfoundation-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `scpfoundation-cli help` (`rpc_interface.txt`).
